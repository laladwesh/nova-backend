...
const TEMPLATE_HEADERS = [
  ...
  "parents", // JSON array, eg. [{"name":"...","email":"...","password":"..."}]
];
...
export function AddEntityPanel({
  ...
  const submitBulk = async () => {
    ...
    if (!res.ok) {
      return alert(
        "Bulk upload failed: " + (body.message || JSON.stringify(body))
      );
    }

    alert(
      `Bulk Upload Complete:\n` +
        `✅ ${body.successCount} succeeded\n` +
  ...
  function downloadTemplate() {
    // convert header row into a one‐row sheet
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === "class") {
      ...
      body = await res.json();
      if (!res.ok) return alert("Error adding class: " + body.message);
      alert("Class created!");
    } else {
      const payload = {
        name: form.name,
      ...
      alert(`${mode.charAt(0).toUpperCase() + mode.slice(1)} added!`);
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-gray-200 rounded-lg px-3 py-2 focus:ring-indigo-500"
              placeholder={`Enter ${mode} name`}
            />
          </div>

          {/* Teacher & Student: email/password */}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {form.parents.map((p, i) => (
                  <div
                    key={i}
                    className="bg-gray-50 p-4 rounded-lg grid grid-cols-1 gap-3 relative"
                  >
                    <button
                      type="button"
          ...
          {mode === "class" && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2 no-scrollbar max-h-32 overflow-auto border border-gray-200 rounded-lg p-2">
                  {teachers.map((t) => (
