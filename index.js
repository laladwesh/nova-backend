...
cron.schedule('*/5 * * * *', async () => {
  try {
    await axios.get('https://ca-portal-39mk.onrender.com');
    console.log(new Date().toISOString(), '› Successfully pinged CA Portal');
  } catch (err) {
    console.error(new Date().toISOString(), '› Error pinging CA Portal:', err.message);
  }
});
